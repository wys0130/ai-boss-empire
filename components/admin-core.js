        ...
        list.forEach((user) => {
            ...
            tbody.innerHTML += `
                <tr class="border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 hover:bg-slate-50 dark:hover:bg-slate-800 transition">
                    ...
                    <td class="py-2 px-2 font-mono text-slate-400 text-xs whitespace-nowrap hidden sm:table-cell">${user.date}</td>
                    <td class="py-2 px-2 text-right whitespace-nowrap">
                        <div class="inline-flex items-center justify-end gap-1 flex-nowrap w-[130px]">
...
// 👑 修复：防报错机制，扩大拉取条数到 30 条，并识别我们自定义的标签
        ...
        commits.forEach((item, idx) => {
            ...
            container.innerHTML += `
                <div class="border rounded-xl p-3 flex items-center justify-between gap-3 saas-input ${bgCls} mb-2 transition hover:shadow-md">
                    ...
                    <button onclick="revertToSelectedCommit('${item.sha}', '${shaShort}')" class="px-3 py-1.5 border border-slate-300 dark:border-slate-600 rounded-lg text-xs font-mono font-bold hover:bg-slate-200 dark:hover:bg-slate-700 transition shrink-0 bg-white dark:bg-slate-800">
                        ${idx === 0 ? '当前状态' : '还原'}
                    </button>
                </div>
            `;
