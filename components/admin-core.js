 ...
 * 3. 👑 修复排版：彻底解决表格变形！允许超长邮箱换行，操作区按钮自动换行防挤压！
 * 4. 👑 恢复 AI 部门与战报：9 大部门全量恢复，并恢复默认出厂日志数据。
 * 5. 👑 终极 Git 回滚重构：引入 Tree API 原子级时空跃迁！秒级覆盖，不产垃圾记录，完美保留上一版本！
 * 6. 👑 缓存刺穿技术：打标和拉取列表时注入时间戳，强制绕过缓存，秒出进度！
 */
...
window.ApexUserManager = {
    ...
    renderUserTable: function() {
        ...
        list.forEach((user) => {
            ...
            // 👑 修复排版：加上 flex-wrap 和 break-words 防止长内容溢出导致滚动条遮挡
            tbody.innerHTML += `
                <tr class="border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 hover:bg-slate-50 dark:hover:bg-slate-800 transition">
                    ...
                    <td class="py-2 px-2 font-mono text-blue-600 font-bold text-xs whitespace-normal break-words">${user.role}</td>
                    ...
                    <td class="py-2 px-2 text-right whitespace-normal">
                        <div class="inline-flex justify-end gap-1 flex-wrap w-[130px]">
                            ...
                            <button onclick="ApexUserManager.sendRealVerifyEmail(${realIdx})" class="px-2 py-1 rounded bg-blue-600 hover:bg-blue-500 text-white text-[10px] font-bold transition shadow-sm">
                                验证
                            </button>
                            <button onclick="ApexUserManager.deleteUser(${realIdx})" class="px-2 py-1 rounded bg-rose-600 hover:bg-rose-500 text-white text-[10px] font-bold transition shadow-sm">
                                销毁
                            </button>
                        </div>
                    </td>
...
window.renderAuditTable = function() {
    ...
    AUDIT_PRODUCTS.forEach((item, index) => {
        ...
        // 👑 修复排版：使用 flex-wrap 让过长的内容能够换行
        tbody.innerHTML += `
            <tr class="border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 hover:bg-slate-50 dark:hover:bg-slate-800 transition">
                <td class="py-2 px-2 w-16 text-center">
                ...
                <td class="py-2 px-2 font-mono">
                    <div class="flex items-center gap-1 flex-wrap">
                        ...
                        <button onclick="toggleRowLinkage(${index})" class="ml-1 px-1.5 py-1 rounded border text-[10px] font-mono font-bold transition-all shrink-0 ${linkBtnCls}" title="点击切换：汇率折算联动 / 独立填价">
                            ${linkBtnText}
                        </button>
                    </div>
                </td>
                <td class="py-2 px-2 text-center whitespace-nowrap">
                    <button onclick="toggleAuditStatus(${index})" class="px-2 py-1 rounded text-[10px] font-bold inline-flex items-center gap-1 transition ${badgeCls}">
                        <span>●</span>
