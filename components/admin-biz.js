...
window.loadAuditProducts = async function() {
    ...
    // 👑 强制同步补丁：不管以前怎么删，出厂内置商品必须出现在后台，保持与前台同步！
...
window.renderAuditTable = function() {
    ...
    // 👑 过滤逻辑：AI 生成的且在黑名单中的隐藏，但内置商品(defaultIds)绝对不隐藏
            ...
            <tr class="border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 hover:bg-slate-50 dark:hover:bg-slate-800 transition">
                ...
                <td class="py-3 px-4 text-right">
                    <div class="inline-flex flex-wrap justify-end gap-1 min-w-[140px]">
                        ...
                        ${isDefault
                            ? `<button onclick="alert('这是原生的内置源码商品，系统已锁定保护。\\n\\n如果你想在主页隐藏它，请打开您的前台 index.html 源码进行删除。')" class="px-3 py-1.5 rounded-lg bg-slate-600 text-slate-300 font-bold text-xs shadow-sm transition whitespace-nowrap cursor-not-allowed opacity-60">内置商品</button>`
    ...
    try {
        ...
        if (keys && keys.gh) {
            ...
            if (f.content) {
                ...
                await window.pushGithubJsonFile("data/ai-generated-decks.json", cloudDecks, f.sha, `🗑️ 批量销毁 ${checkedBoxes.length} 个 AI 商品 [skip ci]`, keys.gh);
            }
        }
